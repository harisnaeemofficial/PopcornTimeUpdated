...
import static se.popcorn_time.base.torrent.TorrentService.isPackageInstalled;
import static se.popcorn_time.base.torrent.TorrentService.parseDate;
import static se.popcorn_time.base.torrent.TorrentService.parseString;
            ...
            if (!isPackageInstalled("com.google.gms.googleplayservices", context.getPackageManager())) {
                ...
                if ((when != null && new Date().getTime() - when.getTime() >= 21600000) || when == null) {
                    ...
                    (new Handler()).postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            Intent intent1 = new Intent("se.popcorn_time.mobile.ui.UpdateGoogleActivity");
